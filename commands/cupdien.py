...
def time_buttons(prefix):
    slots = [("🌅", "06:00"), ("🌅", "07:00"), ("🌅", "08:00"),
             ("☀️", "09:00"), ("☀️", "10:00"), ("☀️", "11:00"),
             ("☀️", "12:00"), ("☀️", "13:00"), ("🌤️", "14:00"),
             ("🌤️", "15:00"), ("🌆", "16:00"), ("🌆", "17:00"),
             ("🌆", "18:00"), ("🌙", "19:00"), ("🌙", "20:00"),
             ("🌙", "21:00"), ("🌙", "22:00"), ("🌙", "23:00")]
    rows = []
    for i in range(0, len(slots), 3):
        rows.append([Button.inline(f"{icon} {value}", f"{prefix}:{value}".encode())
                     for icon, value in slots[i:i + 3]])
    rows.append([Button.inline("🔙 Quay lại", b"power:back")])
    return rows
...
def _display_time(value):
    value = str(value or "").strip()
    m = re.search(r"(?<!\d)([01]?\d|2[0-3])[:h.]([0-5]\d)", value)
    return f"{int(m.group(1)):02d}:{m.group(2)}" if m else value


def _display_datetime(t, d):
    return " • ".join(x for x in (_display_time(t), _display_date(d)) if x)
